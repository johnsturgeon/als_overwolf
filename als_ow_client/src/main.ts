...
function updateGameStateWithInfo(info) {
    let game_state = als.gameStateFromInfo(info)
    let didUpdate = false
    if(game_state) {
...
function registerEvents() {
    ...
    onInfoUpdates2Listener = function(info) {
        let match_event = als.matchEventFromInfo(info)
